...
Implementations can decide for their use-cases when they want a locale to fallback to language or not.
Note: Translations for both language (fallback locale) and the original locale MUST be packed in the same pack file. Please take a look at `pack_id_mapping` property in config.json

For example,
If the application has translations for both `es` and `es-MX` and want to show `es` strings for the ones that are missing in `es-MX`, both `es` and `es-MX` should be packed together
// config.json
pack_id_mapping = {
  "es-rMX": "es"
}
...
There may be some special cases where you may not want the language to be a fallback locale, for example, `zh` should not be a fallback for `zh-TW` because their script are different.
...
Applications should override this method if they support translations for locales with region. For a locale passed in this method, applications should return the locale whose translations should be loaded from the pack file.
In combination with {@code shouldAddLanguageAsParentForLocale(Locale locale)} implementations can define the most significant locale to pick translations of and a broader fallback language, if a translation is not available.

For example,
If an app supports translations for `zh` and `zh-TW` Chinese locales only. So, if the passed in locale to this method is `zh-HK`, you can decide to return `zh-TW`.
